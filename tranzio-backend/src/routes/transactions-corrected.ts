...
import { sendToUser, sendToTransaction } from '../services/socketService';
  ...
  try {
    ...
    try {
      ...
      if (wsService) {
        ...
        wsService.sendToUser(transaction.creatorId, 'transaction:updated', {
          transactionId: transaction.id,
          status: 'WAITING_FOR_SHIPMENT',
          paymentCompleted: true,
        ...
        if (transaction.counterpartyId) {
          wsService.sendToUser(transaction.counterpartyId, 'transaction:updated', {
            transactionId: transaction.id,
            status: 'WAITING_FOR_SHIPMENT',
            paymentCompleted: true,
            message: 'Payment confirmed successfully',
